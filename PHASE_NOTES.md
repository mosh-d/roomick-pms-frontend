...
## Phase 73 — Guest messages and the unified inbox (2026-09-15)

Month 9's last item (see the backend's `PHASE_NOTES.md`): guests message the property from "Manage your booking", and staff read and reply in one inbox.

### Manage your booking — Messages
A section on every looked-up booking. "View messages" loads the conversation — an explicit button, like the bill, because each call spends the shared, throttled guest-credential budget. The guest picks what it's about (a question, a late check-out request, or housekeeping) and writes up to 2,000 characters; their messages and the property's replies show in order, with requests tagged. A checked-in guest's housekeeping request goes straight to housekeeping, and the page says so. Refresh fetches new replies.

### Comms Log — Guest Messages
The inbox sits at the top of the existing Guest Communications Log page rather than as a new sidebar item: the sidebar mirrors the reference exactly, and this page already owns guest communication. One conversation per guest who has written in, newest first, with an "N new" badge, an All / Unread filter, and a 30-second poll. Opening a conversation shows the thread — the guest's messages, staff replies and the automated notices, each labelled Guest / Staff / Automated with its channel — and marks it read (again if a new message arrives while it's open). The reply box defaults to the guest portal and says plainly, per channel, what reaches the guest: the portal immediately; email and SMS are recorded but not delivered until a provider is connected. The reservation timeline further down labels inbound rows "From guest".
