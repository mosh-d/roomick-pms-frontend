...
3. **A card's own 5%/10%/... rest-state tint is Card.tsx's own concern, not touched by this phase's hover/press work.** `HubCard`'s hover (20%) and press (30%) states are fixed absolute values, independent of whatever `CARD_TONE_CLASSES.secondary`'s own rest-state alpha happens to be at any given time.
